...
function persistFixtures(userUid = firedb.app.auth().currentUser.uid) {
  ...
  setFixturesOnBatch(
    TRANSACTIONS_FIXTURE.map((it) => ({ ...it, datetime: new Date(it.datetime) })),
    'transactions'
  );
...
const TAG_COLORS = Object.freeze(['#59BE4A', '#F2D730', '#1277C0']);

const PROJECTS_FIXTURE = [
  { uuid: uuidv4(), name: 'Demonstração', createdAt: new Date().toISOString() },
];
