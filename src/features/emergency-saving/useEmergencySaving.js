...
const useSelectEmergencySaving = () => {
  ...
  const { onlyMonthlyExpenses, onlyMonthlyIncomes, emergencySaving } = monthlySituation;

  const shouldSave =
    onlyMonthlyExpenses.length > 0 && onlyMonthlyIncomes.length > 0 && emergencySaving <= 0;
